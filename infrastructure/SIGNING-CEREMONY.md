...
When re-signing in 90 days for the next quarterly window, copy the most recent ceremony's `sign.py`/`verify.py` (currently `~/llmo-key-ceremony-2026-04-30/`) into a new dated ceremony directory, retarget the `CEREMONY_DIR` constant, and update `valid_from`/`valid_until` and `document_id` in the new payload.
