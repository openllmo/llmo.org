...
For the current v0.1 draft period, decisions are made by the editor with public consultation as appropriate.

**Substantive changes** to normative requirements (new MUST/SHOULD/MAY clauses, changes to the JSON Schema, changes to the trust model, changes to discovery, changes to conformance tiers) require a public discussion period of at least 14 days and solicited implementer feedback before acceptance.

**Editorial revisions** (clarifications, additions to non-normative sections such as §8 Open Questions, wording improvements, typo fixes) may be merged by the editor without the 14-day period. Editorial revisions bump the specification's patch version (e.g., v0.1 to v0.1.1) and are recorded in the [changelog](/spec/changelog).
...
See [CONTRIBUTING.md](https://github.com/openllmo/llmo.org/blob/main/CONTRIBUTING.md) for the workflow. Bug reports, core claim type proposals, and extension claim type registrations each have their own path.
