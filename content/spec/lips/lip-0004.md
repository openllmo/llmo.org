...
The signature MUST verify against the JWK whose thumbprint is recorded in the payload. The registry MUST reject any entry whose signature does not verify against a JWK with the declared thumbprint. This binding closes the loop: only a party holding the private key corresponding to the declared public key can produce a valid entry.
...
2. Extract the signing key's JWK Thumbprint per Section 3.1.
3. Query a conforming KT registry: `GET /kt/v1/entries?domain=<domain>`.
4. Verify that at least one returned entry has a payload whose `jwk_thumbprint` matches the thumbprint computed in step 2 AND whose JWS signature verifies against that key.
5. If yes, evaluate X7 as PASS. If no, evaluate X7 as FAIL with note `kt_uninlogged`.
