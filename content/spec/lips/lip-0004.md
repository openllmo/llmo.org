...
transitions:
  - date: 2026-05-12
    from: null
    to: Draft
    ref: "Initial draft, PR #138"
  - date: 2026-05-12
    from: Draft
    to: Final
    ref: "Accepted under editor authorship privilege during the pre-announcement Goldilocks period. The LIP-1 §10 14-day governance window is observed informally for this transition; full window observance resumes for substantive normative changes after the v0.1 public announcement. The 90-day post-Final grace period for X7 enforcement originally specified in §5 is revised in the same PR to 14 days, reflecting that pre-launch there are zero or very few existing publishers requiring the grace."
---
...
- A grace period of 14 days from the LIP's Final status to the date at which conforming consumers begin enforcing X7. During the grace period, the `kt_uninlogged` note is surfaced advisorily. The original drafting of this LIP specified a 90-day grace period; the period was revised down to 14 days at the same time the LIP transitioned to Final, reflecting that pre-launch there are zero or very few existing publishers needing protection from the X7 transition. Future revisions of this LIP MAY extend the grace period if observed adoption produces a confused-publisher class that would benefit from a longer window.
